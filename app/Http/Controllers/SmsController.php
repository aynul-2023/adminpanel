...
class SmsController extends Controller
{
    ...
    {
        //'api_key' => config('app.vonage_api_key');
        //'api_secret' => config('app.vonage_api_secret');
        $NEXMO_API_KEY = 'e76a24ad';
        $NEXMO_API_SECRET = 'C6rU6BPDVQMAJ0la';
        $SENDER_ID = '8801976355569';

        $basic = new \Vonage\Client\Credentials\Basic($NEXMO_API_KEY, $NEXMO_API_SECRET);
        $client = new \Vonage\Client($basic);

        $response = $client->sms()->send(
            new \Vonage\SMS\Message\SMS($request->to_number, $SENDER_ID, $request->message)
        );
